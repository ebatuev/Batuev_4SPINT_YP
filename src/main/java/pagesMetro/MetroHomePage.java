...
public class MetroHomePage {
    ...
    private final By routeListItenTime = By.className("route-list-item__time");
    // локатор коллекций станций «Откуда» и «Куда» маршрута по имени класса
    ...
    public void waitForLoadHomePage() {
        ...
        new WebDriverWait(driver, 8)    // Чтобы проверить время ожидания, нужна проверка с использованием класса WebDriverWait. Добавь ожидание видимости этого элемента восемь секунд
    ...
    public void chooseCity(String cityName) {
        ...
        // выбор города
        driver.findElement(By.xpath(String.format("//*[text()='%s']", cityName))).click(); // Обрати внимание: здесь можно использовать метод format класса String. С ним проще вставить строку с именем города cityName в локатор.
    }

    // метод ввода названия станции в поле «Откуда»
    public void setStationFrom(String stationFrom) { // Указание станции «Откуда» будет таким: setStationFrom(stationFrom)
        // ввели название станции в поле ввода, а затем с помощью клавиш «Вниз» и Enter выбери его в выпадающем списке саджеста
        driver.findElement(fieldFrom).sendKeys(stationFrom, Keys.DOWN, Keys.ENTER); // чтобы ввести в поле элемента значение, используй метод sendKeys(). В параметры передай все значения, которые нужны для ввода: stationFrom, Keys.DOWN, Keys.ENTER.
    }
    ...
    // метод построения маршрута - объединили 3 метода в один
    ...
    public String getApproximateRouteTime(int routeNumber) {
        ...
        return driver.findElements(By.className("route-list-item__time")).get(routeNumber).getText(); // Теперь верни текст требуемого элемента из коллекции, в которой хранятся расчёты примерного времени для маршрутов. Сначала найди эту коллекцию с помощью локатора By.className("route-list-item__time"). Далее методом get() возьми элемент с номером, переданным в параметр. Затем верни его текст с помощью метода getText().
    }

    // метод проверки с ожиданием видимости станции метро
    public void waitForStationVisibility(String stationName) {
        new WebDriverWait(driver, 8)
                .until(ExpectedConditions.visibilityOfElementLocated(By.xpath(String.format("//*[text()='%s']", stationName)))); // Для метода с ожиданием видимости некоторой станции метро waitForStationVisibility, используй тот же подход, что и при поиске города.
    }
}
